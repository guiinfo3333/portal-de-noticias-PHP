...
<div class="principal">
	...
	<header>
		...
		<h5 id="subtitulo5"><?php echo "$dta[2]/$dta[1]/$dta[0]";?></h5>
	</header>
<hr></hr>
...
<div class="news2">
    ...
    <ul>
        ...
        <?php
        include 'connection.php';
              $res= mysqli_query($con,"SELECT * FROM noticia ORDER BY RAND() LIMIT 3;");
              $oi= mysqli_affected_rows($con);
              while($vreg= mysqli_fetch_row($res)){
      echo utf8_encode("<li><a href='secundarianoti.php?id=$vreg[0]'>$vreg[6]</a></li>");
    }
      ?> 
...
<div class="news3"  style="top:150px;">
...
<!--======================botaoo par visualizar=================-->
<?php
 $_SESSION['idnoticia']=$id;
 $idusuario=$_SESSION['id'];
 if (isset($_SESSION['id'])) {
 include 'connection.php';
 $sql="select * from ler where idusuario='$idusuario' and idnoticia='$id';";
 $res=mysqli_query($con,$sql);
     $ca=mysqli_affected_rows($con);
        if ($ca>0) {
           echo "<script>alert('Você marcou como lido esta notícia');</script>";
        }else{
          echo "
<form name='hoje' id='mais1' action='views+1.php' method='POST'>
  <input type='checkbox' name='checkview' id='' style='display:none;' value='$id' checked>
  <input type='submit' name='views+1' value='Marcar como Lido' id='mais2' onclick='paraanima()'/>
</form>";
 }
        }
